...
function Row({ Icon, label }: { Icon: typeof BellIcon; label: string }) {
  return (
    <button className="flex items-center gap-3 border-b border-linha py-4 text-left last:border-none">
      ...
      <span className="flex-1 text-[15px] text-marfim">{label}</span>
...
export function Profile() {
  return (
    <div className="flex flex-col gap-6 px-5 pb-8 pt-5">
      <h1 className="font-display text-[24px] font-semibold text-marfim">Perfil</h1>

      <div className="flex items-center gap-4">
        <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-ambar font-display text-[22px] font-semibold text-breu">
          M
        </div>
        <div className="flex flex-col">
          <span className="text-[16px] font-medium text-marfim">Mariana</span>
          <span className="text-[13px] text-areia">mariana@email.com</span>
        </div>
      </div>
